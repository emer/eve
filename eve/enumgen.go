...
var _NodeTypesNameToValueMap = map[string]NodeTypes{`BODY`: 0, `GROUP`: 1, `JOINT`: 2}
...
var _NodeFlagsNameToValueMap = map[string]NodeFlags{`Dynamic`: 7}

var _NodeFlagsDescMap = map[NodeFlags]string{7: `Dynamic means that this node can move -- if not so marked, it is a Static node. Any top-level group that is not Dynamic is immediately pruned from further consideration, so top-level groups should be separated into Dynamic and Static nodes at the start.`}

var _NodeFlagsMap = map[NodeFlags]string{7: `Dynamic`}
